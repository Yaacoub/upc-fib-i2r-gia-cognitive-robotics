...
from Pipelines.ACTR.model import ActRModel
...
def _execute_script(command_id: CommandID, command: Command, ground_truth: GroundTruth, action_sequence: ActionSequence, script: Script, token_usage: TokenUsage, parse_time: ParseTime) -> PipelineResult:
    chunk_file_path = Path(__file__).resolve().parent / "cognitive-robotics.actr.json"

    actr_start = time.perf_counter()
    model = ActRModel.from_chunk_file(str(chunk_file_path))
    trace_output = model.run(action_sequence)
    actr_time = time.perf_counter() - actr_start

    error_type = None
    success = False

    if model.failure_reason is not None:
        error_type = map_error_type(model.failure_reason)
    else:
        success = True
    ...
    return PipelineResult(
        ...
        metrics=ExecutionMetrics(
            ...
            symbolic_reasoning_time=actr_time,
